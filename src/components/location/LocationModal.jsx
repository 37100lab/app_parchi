...
export const LocationModal = ({ loc }) => {
  ...
  if(
     (
      loc.properties &&
      ( !('denominazi' in loc.properties) || !('nome_via' in loc.properties)||!('circoscriz' in loc.properties)
      || !('tipo_area' in loc.properties)|| !('sup_verde' in loc.properties)) || !('sup_pavim' in loc.properties)
     )
     || !loc.properties
    )
        return(
  ...
  return (
    ...
    <IonList>
      <IonItem>
        ...
        <IonLabel className="ion-text-wrap">{stringManager.titleCase(loc.properties.nome_via)}</IonLabel>
      </IonItem>
      <IonItem>
      ...
      <IonItem>
        <IonNote slot="start" color="primary">Sup. Verde</IonNote>
        <IonLabel>{controlValue(loc.properties.sup_verde)}</IonLabel>
      </IonItem>
      <IonItem>
        <IonNote slot="start" color="primary">Sup. Pavim.</IonNote>
